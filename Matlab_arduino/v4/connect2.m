...
% Last Modified by GUIDE v2.5 27-Jan-2022 17:21:20
...
%yline graphics
   ...
   %yline(ref_rpm,'--r','LineWidth',3) %referans rpm
   plot([0,30],[ref_rpm,ref_rpm],'--r');
   hold on;
   plot([0,1],[ref_rpm+20,ref_rpm+20],'--w');
   hold on;
%x graph variables   
rpmsample=0
x=0
...
%rpm graphs
    x=[x rpm];
    motor=plot(x,'-b','LineWidth',3);
    
    disp("rpmsample: "+rpmsample)
    xlabel('\bfTime(t*2)')
    ylabel('\bfRPM Value') 
    grid on;
    legend('ref','motor','Location','southeast')
    rpmsample=rpmsample+1
...
%     end
    ...
    if (-ref_rpm+rpm>30)
        ...
        continue
         ...
         if (ref_rpm-rpm>10) % daha az fark
        ePin1=ePin1+0.03;
        writePWMVoltage(a,'D5',ePin1);
        continue
    end
