...
from tkinter import *
root =Tk() #first thing
root.title("Müşteri Sipariş Arayüzü")
...
def blokekle():
    ...
    transaction= f"\"{musteri}\" id numaralı müşteri, \
\"{satici}\" id numaralı satıcıdan {yumurta} adet \
yumurta satın aldı."
    ...
    try:
        totalprice="%.2f TL" %(1.8*int(yumurta))
        total.delete("1.0",END)
        total.insert(INSERT,totalprice)
    except:
        total.delete("1.0",END)
        total.insert(INSERT,"Hatalı Yumurta Sayısı")
...
total=Text(root,height=1, width=30,padx=5)
total.grid(row=5,column=1,padx=10,pady=5,columnspan=1 )

blok=Text(root,height=5, width=40,padx=5)
blok.grid(row=6,column=1,padx=1,pady=5,columnspan=3 )

hashcode=Text(root,height=2, width=40,padx=5)
hashcode.grid(row=7,column=1,padx=1,pady=5,columnspan=4 )

# text box label

musteri_id_label=Label(root,text="Müsteri ID:",pady=10)
musteri_id_label.grid(row=0,column=0)

satici_id_label=Label(root,text="Satıcı ID:",pady=10)
satici_id_label.grid(row=1,column=0)

yumurta_sayisi_label=Label(root,text="Yumurta adeti:",pady=10)
yumurta_sayisi_label.grid(row=2,column=0)

total_label=Label(root,text="Sipariş Tutarı:",pady=1)
total_label.grid(row=5,column=0)

blok_label=Label(root,text="Blok verisi",pady=1)
blok_label.grid(row=6,column=0)

hashcode_label=Label(root,text="Hash Kodu",pady=1)
hashcode_label.grid(row=7,column=0)


# create buy button

satınal=Button(root,text="Satın Al",command=blokekle)
